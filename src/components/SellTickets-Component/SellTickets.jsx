...
class SellTickets extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            sellTicketsDetails:{
                SeatSection:'',
                SeatRow:'',
                SeatNumber:'', // ticket quantity not mat
                TicketNotes:'',
                Currency:'',
                TicketOriginalPrice:'',
                SellerSellingPrice:'',
    ...
    componentDidMount() {
        window.scrollTo(0, 0)
        // console.log(this.props.location, this.state.eventId, this.state.selectedEvent)
        if (!this.props.loggedIn) {
            ...
            if(this.props.location.state && this.props.location.state.eventDetail && this.props.location.state.eventId){
                // console.log(sessionStorage.getItem("isTicketUploaded"))
                // if(sessionStorage.getItem("isTicketUploaded")){
                //     this.props.location.state = null
                // }else{
                    this.onSelectEvent(this.props.location.state.eventId, {...this.props.location.state.eventDetail})
                // }
            }
        }
    }
    ...
    handleValidation = () =>{
        ...
        if(sellTicketsDetails['TicketNotes']){
            Valid = true 
        }else{
            return Valid = false;
        }
                                ...
                                <Row>
                                    ...
                                    <Col sm={12} md={8} lg={8}>
                                        ...
                                        <Form.Group style={{display:'block'}}>
                                            ...
                                            <Form.Control type="textarea" name="TicketNotes" onChange={(e) => this.handleTicketDetailInput('TicketNotes', e)} value={this.state.sellTicketsDetails['TicketNotes']}/>
                                        </Form.Group>
                                    </Col>
                                    }
                                </Row>
    ...
    finishProcess = async () => {
        ...
        console.log("Ready Data:", data, "initial State", this.initialState)
        ...
        if(this.props.listingResponse.sellticket.response){
            // if(this.props.location.state && this.props.location.state.eventDetail && this.props.location.state.eventId){
            //     sessionStorage.setItem("isTicketUploaded", true);
            // }
            this.setState({
            ...
            return (
                <Container>
                    <Row className="justify-content-md-center">
                        ...
                        <div className="buttons-group d-flex justify-content-between align-items-center mt-3">
                            <Button type="submit" variant="secondary" className="py-2" onClick={() => this.resetStepForm()}> List Another Ticket </Button>
