...
import BuildVenueMap from '../SeaticsMap/BuildVenueMap'
...
class EventSeatsTicketsPage extends Component {
    constructor(props){
        ...
        this.state = {
            ...
            errorMessage:'',
            openInfo:false,
            toxonomy:props.match.params.eventCategory,
            TicketData:[],
        }
    }
    componentDidMount(){
        ...
        // setTimeout(function(){  
        //     Seatics.addTicketData(demoTicketData); 
    ...
    getEventDetails = async (eventId)=>{
        ...
        // console.log(this.props.eventDetail)
        if(this.props.eventDetail){
            var TicketData = this.props.eventDetail.ticketData.map((element,i )=> {
                            var value = [];
                            value.push(element.AdditionalTicketInfo)
                            let properties = {
                            // Seller:element.Seller,
                            tgUserSec:element.SeatSection, 
                            tgUserRow:element.SeatRow, 
                            // tgUserSeats:"1-2",
                            // tgQty:element.SeatNumber, 
                            tgQty:2, 
                            tgPrice:element.SellerSellingPrice.toFixed(2), 
                            tgID:element._id, 
                            tgNotes:element.TicketNotes, 
                            tgMark:1, 
                            tgType:1, 
                            tgClientData:element.Seller, 
                            tgCType:element.TicketType,
                            tgDisclaimers:value
                            };
                            return properties;
                        });
    ...
    render(){
        const { show, loader, event, venue, performers, recomended , eventTicket, errorMessage, openInfo,toxonomy, TicketData} =  this.state
        // const tickets_count = {...event.stats}
        // console.log("Seats of Events", event)
        // console.log("Seats of Events", recomended)
        // console.log('Seatics',window.Seatics)
        return (
            <>
                            ...
                            <Col sm={12}>
                                {
                                    loader 
                                    ?
                                        <Loader type="ThreeDots" color="#e94d13" height={50} width={30} timeout={3000} />
                                    :
                        ...
                        <Container fluid>
                            <Row className="align-items-center">
                                <Col sm={12} lg={6}>
                                    ...
                                    {/* <Button role="button" onClick={()=> this.props.history.push(`/event/${toxonomy}`)} variant="outline-primary">Back</Button> */}
                                    </>
                                    }
                                </Col>
                                <Col sm={12} lg={6}>
                ...
                <section className="mapMain">
                    <Container fluid>
                        {!Object.values(event).every(x => (x === null || x === '')) 
                            ? <>
                                <BuildVenueMap style={{textAlign:'center', textTransform: 'uppercase'}} event={event} venue={venue} TicketData={TicketData}/>
                                </>
                                
                            :
                                <h4 style={{textAlign:'center', textTransform: 'uppercase'}}>Loading Map</h4>
                        }
                        {/* <Row>
                            ...
                            <Col sm={12} lg={6}>
                                <h4 style={{textAlign:'center', textTransform: 'uppercase'}} >Map Comming Soon</h4>
                                <div className="mapArea">
                                   {!Object.values(event).every(x => (x === null || x === '')) && <BuildVenueMap style={{textAlign:'center', textTransform: 'uppercase'}} event={event} venue={venue} performers={performers}/>}
                                </div>
                            </Col>
                        </Row> */}
                        {/* {console.log('Seatics',window.Seatics)} */}
